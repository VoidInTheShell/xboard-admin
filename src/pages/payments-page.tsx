...
export function PaymentsPage() {
  ...
  const [secretBaseline, setSecretBaseline] = React.useState<
  ...
  async function loadSchema(method: string, id?: number) {
    ...
    setFormErrors({})
    try {
      const next = await api.post<Record<string, PaymentField>>(
  ...
  function openEdit(payment: Payment) {
    setForm({
      ...
      handlingFeeFixed: payment.handling_fee_fixed
        ? payment.handling_fee_fixed / 100
        : '',
      handlingFeePercent: payment.handling_fee_percent ?? '',
      values: {},
    })
    setSchema({})
    setSecretBaseline({})
  ...
  async function savePayment() {
    ...
    const config = Object.fromEntries(
      Object.keys(schema).map((key) => [
        key,
        isSensitiveField(key, schema[key]) && !form.values[key]
          ? (secretBaseline[key] ?? '')
          : (form.values[key] ?? ''),
      ]),
    )
    try {
      await api.post<boolean>('payment/save', {
        ...(form.id ? { id: form.id } : {}),
        name: form.name.trim(),
        ...
        handling_fee_fixed:
          form.handlingFeeFixed === ''
            ? null
            : Math.round(Number(form.handlingFeeFixed) * 100),
        handling_fee_percent:
          form.handlingFeePercent === ''
            ? null
            : Number(form.handlingFeePercent),
      })
  ...
  async function movePayment(index: number, offset: -1 | 1) {
    ...
    setSaving(true)
    try {
      await api.post<boolean>('payment/sort', {
        ids: next.map((payment) => payment.id),
      })
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px]">
      <PageHeader
        title="支付管理"
        description="配置支付渠道、手续费和展示顺序。密钥不会回显，留空可保留已保存的值。"
        action={
          <Button onClick={openCreate} disabled={!methods.length}>
      ...
      <Card className="gap-0 overflow-hidden py-0 shadow-none">
        <div className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="pl-4">支付方式</TableHead>
                <TableHead>网关</TableHead>
                <TableHead>手续费</TableHead>
                <TableHead>通知域名</TableHead>
                  ...
                  <TableRow key={payment.id}>
                    <TableCell className="pl-4">
                      <div className="flex items-center gap-2">
                        <span className="grid size-8 place-items-center rounded-lg border bg-muted text-sm">
                          {payment.icon || (
                    ...
                    <TableCell className="font-data text-xs">
                      <div>
                        固定 ¥
                        {((payment.handling_fee_fixed ?? 0) / 100).toFixed(2)}
                      </div>
                      <div className="text-muted-foreground">
                        比例 {Number(payment.handling_fee_percent ?? 0)}%
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex flex-col items-start gap-1">
                        <span className="text-xs">
                          {safeOrigin(payment.notify_domain) || '使用站点域名'}
                        </span>
                        <Button
                          variant="link"
                          size="sm"
                          className="h-auto px-0 text-xs"
                          disabled={!payment.notify_url}
                          onClick={() => void copyNotifyUrl(payment)}
                        >
                          <Clipboard
                    ...
                    <TableCell>
                      <StatusBadge
                        ...
                        tone={payment.enable ? 'success' : 'neutral'}
                      />
                    </TableCell>
                    <TableCell>
                      <ButtonGroup aria-label={`${payment.name} 排序`}>
                        <Button
                          variant="outline"
                          size="icon-sm"
                          aria-label={`上移 ${payment.name}`}
                          disabled={saving || index === 0}
                          onClick={() => void movePayment(index, -1)}
                        >
                        ...
                        <Button
                          variant="outline"
                          size="icon-sm"
                          aria-label={`下移 ${payment.name}`}
                          disabled={saving || index === payments.length - 1}
                          onClick={() => void movePayment(index, 1)}
                        >
                      ...
                      >
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuGroup>
                              <DropdownMenuItem
                                onSelect={() => setToggleTarget(payment)}
                              >
                                {payment.enable ? '停用' : '启用'}
                              </DropdownMenuItem>
                            </DropdownMenuGroup>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem
        ...
        {!query.loading && !payments.length ? (
          <ResourceEmpty
            title="还没有支付方式"
            description={
              methods.length
                ? '从已启用的支付插件中选择网关并填写配置。'
                : '当前没有已启用且可提供支付网关的插件，请先在扩展管理中启用支付插件。'
            }
            action={
              methods.length ? (
        ...
        <div className="border-t px-4 py-3 text-xs text-muted-foreground">
          共 {payments.length} 个支付方式 · 完整回调 UUID
          仅在主动复制时进入剪贴板
        </div>
      </Card>
      ...
      <PaymentDialog
        ...
        onMethodChange={(method) => {
          setForm((current) =>
            current ? { ...current, method, values: {} } : current,
          )
          void loadSchema(method)
        }}
        onSave={savePayment}
      />
      <ConfirmActionDialog
        ...
        title={`${toggleTarget?.enable ? '停用' : '启用'}支付方式“${toggleTarget?.name ?? ''}”？`}
        description={
          toggleTarget?.enable
            ? '停用后用户端不再展示该支付入口，正在处理的第三方回调仍可能到达。'
            : '启用后用户可立即选择该网关支付，请先确认配置与回调地址有效。'
        }
        confirmLabel={toggleTarget?.enable ? '确认停用' : '确认启用'}
        destructive={Boolean(toggleTarget?.enable)}
        busy={saving}
        onConfirm={togglePayment}
      />
      <ConfirmActionDialog
        open={Boolean(deleteTarget)}
        onOpenChange={(open) => !open && setDeleteTarget(null)}
        title={`删除支付方式“${deleteTarget?.name ?? ''}”？`}
        description="这会删除网关配置与回调 UUID，后续回调将无法匹配该支付方式。历史订单记录不会因此改写。"
        confirmLabel="删除支付方式"
        destructive
        busy={saving}
...
function PaymentDialog({
  ...
  setForm: React.Dispatch<React.SetStateAction<PaymentForm | null>>
  schema: Record<string, PaymentField>
  setSchemaValues: (key: string, value: string) => void
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && !saving && setForm(null)}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle>{form.id ? '编辑支付方式' : '新增支付方式'}</DialogTitle>
          <DialogDescription>
            按支付渠道要求填写配置。密钥、令牌和私钥不会回显；留空可保留已保存的值。
          </DialogDescription>
        </DialogHeader>
        <div className="min-h-0 overflow-y-auto pr-1">
          <FieldGroup>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              />
              <Field>
                <FieldLabel htmlFor="payment-method">支付网关</FieldLabel>
                <Select
                  value={form.method}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <TextField
                ...
                errors={formErrors.notify_domain}
                type="url"
                description="留空使用站点地址；只填写域名与协议。"
                onChange={(notifyDomain) => setForm({ ...form, notifyDomain })}
              />
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <NumberField
                id="payment-fixed"
                label="固定手续费"
                value={form.handlingFeeFixed}
                min={0}
                step="0.01"
                description="站点货币主单位。"
                onChange={(handlingFeeFixed) =>
                  setForm({ ...form, handlingFeeFixed })
                }
              />
              <NumberField
                id="payment-percent"
                label="比例手续费（%）"
                value={form.handlingFeePercent}
                min={0}
                max={100}
                step="0.01"
                onChange={(handlingFeePercent) =>
                  setForm({ ...form, handlingFeePercent })
                }
              />
            </div>
            <FieldSet className="rounded-2xl border p-4">
              <FieldLegend>网关配置</FieldLegend>
              {schemaLoading ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {Object.entries(schema).map(([key, field]) => (
                    <DynamicField
                      ...
                      onChange={(value) => setSchemaValues(key, value)}
                    />
                  ))}
                </div>
              )}
        ...
        <DialogFooter>
          ...
          <Button
            disabled={
              saving || schemaLoading || !form.name.trim() || !form.method
            }
            onClick={onSave}
          >
            {saving ? (
...
function DynamicField({
  ...
  if (options.length)
    return (
      <Field>
        <FieldLabel htmlFor={`payment-config-${fieldKey}`}>
          {field.label || fieldKey}
        </FieldLabel>
        <Select value={value} onValueChange={onChange}>
          <SelectTrigger id={`payment-config-${fieldKey}`} className="w-full">
...
function isSensitiveField(key: string, field: PaymentField) {
