...
type ClientPayload = {
  clients: Client[]
  platform_defaults?: PlatformDefault[]
  templates: string[]
...
export function ClientsPage() {
  ...
  const load = React.useCallback(
    (signal: AbortSignal) =>
      api.get<ClientPayload>('client/fetch', undefined, signal),
    [api],
  )
  ...
  const [defaultSavingKey, setDefaultSavingKey] = React.useState<string | null>(
    null,
  )
  ...
  const selection = useListSelection(clients)

  async function setClientEnabled(item: Client, enabled: boolean) {
    const latest = (await api.get<ClientPayload>('client/fetch')).clients.find(
      (client) => client.id === item.id,
    )
    if (!latest) throw new Error('客户端已不存在，请刷新列表。')
    await api.post<Client>('client/save', {
      id: latest.id,
      name: latest.name,
      description: latest.description,
      logo_mode: latest.logo_mode,
      ...(latest.logo_mode === 'url' ? { logo_url: latest.logo_url } : {}),
      tags: latest.tags,
      download_url: latest.download_url,
      docs_url: latest.docs_url,
      quick_import_enabled: latest.quick_import_enabled,
      quick_import_url: latest.quick_import_url,
      subscription_template: latest.subscription_template,
      scopes: latest.scopes.map(({ device_type, platform }) => ({
        device_type,
        platform,
      })),
      is_enabled: enabled,
  ...
  function openCreate() {
    ...
    setForm({
      ...
      quickImportUrl: '',
      subscriptionTemplate: templates[0] ?? 'clashmeta',
      scopes: [],
      isEnabled: true,
    })
  }
  ...
  async function saveClient() {
    ...
    body.set(
      'quick_import_url',
      form.quickImportEnabled ? form.quickImportUrl.trim() : '',
    )
  ...
  async function saveSort() {
    setSaving(true)
    try {
      await api.post<boolean>('client/sort', {
        device_type: sortDevice,
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px]">
      <PageHeader
        title="客户端适配"
        description="维护用户端下载入口、订阅模板、快速导入 Scheme、平台展示范围及默认推荐客户端。"
        action={
          ...
          <div className="flex flex-col gap-2 border-b px-5 py-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                为每个设备与系统平台选择一个默认客户端；清除手动选择后自动按平台排序回退。
              </p>
            </div>
          ...
          <div className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-3">
            {Object.entries(devicePlatforms).flatMap(([device, platforms]) =>
              platforms.map((platform) => {
                ...
                const configured = platformDefaults.find(
                  (item) =>
                    item.device_type === device && item.platform === platform,
                )
                const fallback = options[0]
                const selectedValue =
                  configured?.is_manual && configured.client_app_id !== null
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3">
                      <Select
                        value={selectedValue}
                        disabled={!options.length || defaultSavingKey === key}
                        onValueChange={(value) =>
                          void savePlatformDefault(device, platform, value)
                        }
                      >
                        <SelectTrigger className="w-full">
                          <SelectValue placeholder="选择默认客户端" />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectGroup>
      ...
      <Card className="gap-0 overflow-hidden py-0 shadow-none">
            ...
            actions={[
              {
                id: 'enable',
                label: '启用所选客户端',
                description: '在用户端显示所选客户端的下载和导入入口。',
                icon: Power,
                run: (item) => setClientEnabled(item, true),
              },
              {
                id: 'disable',
                label: '停用所选客户端',
                description:
                  '隐藏所选客户端入口；平台默认推荐会改用其他可用客户端。',
                icon: PowerOff,
                run: (item) => setClientEnabled(item, false),
              },
              {
                id: 'delete',
                label: '删除所选客户端',
                description: '删除所选客户端入口，并从平台推荐中移除。',
                destructive: true,
                icon: Trash2,
                run: (item) =>
                  api.post<boolean>('client/drop', { id: item.id }),
              },
            ]}
          />
        </div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>订阅模板</TableHead>
                <TableHead>设备平台</TableHead>
                <TableHead>快速导入</TableHead>
                <TableHead>下载 / 文档</TableHead>
              ...
              {query.loading ? (
                ...
                clients.map((client) => (
                  <TableRow
                    key={client.id}
                    data-state={
                      selection.selectedIds.has(client.id)
                        ? 'selected'
                        : undefined
                    }
                  >
                      ...
                      <div className="flex flex-wrap gap-1">
                        {client.scopes.map((scope) => (
                          <Badge
                            ...
                            variant="outline"
                          >
                            {platformLabels[scope.platform] ?? scope.platform}
                          </Badge>
                        ))}
                      </div>
                    </TableCell>
                    <TableCell>
                      <StatusBadge
                        label={
                          client.quick_import_enabled ? '已启用' : '未启用'
                        }
                        tone={
                          client.quick_import_enabled ? 'success' : 'neutral'
                        }
                      />
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Badge
                        variant={client.is_builtin ? 'secondary' : 'outline'}
                      >
                        {client.is_builtin ? '内置' : '自定义'}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <ButtonGroup
                        className="ml-auto"
                        aria-label={`${client.name} 操作`}
                      >
                        <Button
                          variant="outline"
                          size="sm"
        ...
        {!query.loading && !clients.length ? (
          <ResourceEmpty
            title="还没有客户端入口"
            description="新增后可在用户端按设备平台展示下载和快速导入入口。"
            action={
        ...
        <div className="border-t px-4 py-3 text-xs text-muted-foreground">
          共 {clients.length} 个客户端 · 平台排序独立维护
        </div>
      </Card>
      ...
      <Dialog
        open={sortOpen}
        onOpenChange={(open) => !saving && setSortOpen(open)}
      >
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>调整平台内客户端顺序</DialogTitle>
            <DialogDescription>
              按设备与系统平台分别调整客户端顺序；保存时会更新当前平台的完整排序。
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 sm:grid-cols-2">
            <Field>
              <FieldLabel htmlFor="sort-device">设备类型</FieldLabel>
              <Select
                value={sortDevice}
                onValueChange={(device) =>
                  changeSortScope(device, devicePlatforms[device]?.[0] ?? '')
                }
              >
                <SelectTrigger id="sort-device" className="w-full">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectGroup>
                    {Object.keys(devicePlatforms).map((device) => (
            ...
            <Field>
              <FieldLabel htmlFor="sort-platform">系统平台</FieldLabel>
              <Select
                value={sortPlatform}
                onValueChange={(platform) =>
                  changeSortScope(sortDevice, platform)
                }
              >
                <SelectTrigger id="sort-platform" className="w-full">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectGroup>
                    {(devicePlatforms[sortDevice] ?? []).map((platform) => (
          ...
          <div className="space-y-2">
            {sortIds.map((id, index) => {
              ...
              return (
                ...
                >
                  <span className="font-data w-6 text-center text-xs text-muted-foreground">
                    {index + 1}
                  </span>
                  ...
                  <Button
                    variant="ghost"
                    size="icon-sm"
                    aria-label={`上移 ${client.name}`}
                    disabled={index === 0}
                    onClick={() => setSortIds(move(sortIds, index, -1))}
                  >
                  ...
                  <Button
                    variant="ghost"
                    size="icon-sm"
                    aria-label={`下移 ${client.name}`}
                    disabled={index === sortIds.length - 1}
                    onClick={() => setSortIds(move(sortIds, index, 1))}
                  >
          ...
          <DialogFooter>
            ...
            <Button
              disabled={saving || !sortIds.length}
      ...
      <ConfirmActionDialog
        ...
        title={`删除客户端“${deleteTarget?.name ?? ''}”？`}
        description="该入口会从用户端所有设备平台移除；如果使用上传 Logo，对应文件也会一并删除。此操作不影响用户订阅本身。"
        confirmLabel="删除客户端"
        destructive
        busy={saving}
...
function ClientDialog({
  ...
  templates: string[]
  devicePlatforms: Record<string, string[]>
  saving: boolean
  onSave: () => void
}) {
  if (!form) return null
  const hasScope = (device: string, platform: string) =>
    form.scopes.some(
      (scope) => scope.device_type === device && scope.platform === platform,
    )
  const toggleScope = (device: string, platform: string, checked: boolean) =>
    setForm({
      ...form,
      scopes: checked
        ? [...form.scopes, { device_type: device, platform }]
        : form.scopes.filter(
            (scope) =>
              scope.device_type !== device || scope.platform !== platform,
          ),
    })
  return (
      ...
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-4xl">
        ...
        <FieldGroup>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            />
            <Field>
              <FieldLabel htmlFor="client-template">订阅模板</FieldLabel>
              <Select
                value={form.subscriptionTemplate}
                onValueChange={(subscriptionTemplate) =>
                  setForm({ ...form, subscriptionTemplate })
                }
              >
                <SelectTrigger id="client-template" className="w-full">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectGroup>
          ...
          <Field data-invalid={Boolean(errors.description)}>
            <FieldLabel htmlFor="client-description">描述</FieldLabel>
            <Textarea
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <TextField
              ...
              label="文档地址"
              value={form.docsUrl}
              errors={errors.docs_url}
              type="url"
              onChange={(docsUrl) => setForm({ ...form, docsUrl })}
            />
          </div>
          <Field>
            <FieldLabel htmlFor="client-tags">标签</FieldLabel>
            <Input
              id="client-tags"
              value={form.tags}
              placeholder="推荐, Android"
              onChange={(event) =>
          ...
          <FieldSet className="rounded-2xl border p-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field>
                <FieldLabel htmlFor="client-logo-mode">来源</FieldLabel>
                <Select
                  value={form.logoMode}
                  onValueChange={(logoMode) =>
                    setForm({
                      ...form,
                      logoMode: logoMode as 'upload' | 'url',
                      logoFile: null,
                    })
                  }
                >
                  <SelectTrigger id="client-logo-mode" className="w-full">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
                      <SelectItem value="url">远程 URL</SelectItem>
              ...
              {form.logoMode === 'url' ? (
                ...
                <Field data-invalid={Boolean(errors.logo_file)}>
                  <FieldLabel>图片文件</FieldLabel>
                  <ClientLogoUpload
                    file={form.logoFile}
                    currentUrl={form.currentLogoUrl}
                    disabled={saving}
                    onChange={(logoFile) => setForm({ ...form, logoFile })}
                  />
                  <FieldDescription>
                    裁剪后预览，保存客户端时上传。未替换时保留已有图标。
                  </FieldDescription>
                  <FieldError
                    errors={errors.logo_file?.map((message) => ({ message }))}
                  />
                </Field>
              )}
            </div>
          ...
          <Field orientation="horizontal" className="rounded-2xl border p-4">
            <span>
              <FieldLabel htmlFor="client-enabled">客户端状态</FieldLabel>
              <FieldDescription>
                停用后不会出现在用户端；平台默认会自动回退到其他已启用客户端。
              </FieldDescription>
            </span>
            <Switch
              id="client-enabled"
              checked={form.isEnabled}
              onCheckedChange={(isEnabled) => setForm({ ...form, isEnabled })}
            />
          </Field>
          <Field orientation="horizontal" className="rounded-2xl border p-4">
            <span>
              <FieldLabel htmlFor="client-quick">快速导入</FieldLabel>
              <FieldDescription>
                启用后可从用户端拉起客户端并注入订阅地址。
              </FieldDescription>
            </span>
            <Switch
              id="client-quick"
              checked={form.quickImportEnabled}
              onCheckedChange={(quickImportEnabled) =>
                setForm({ ...form, quickImportEnabled })
              }
            />
          </Field>
          {form.quickImportEnabled ? (
            <TextField
              id="client-quick-url"
              label="快速导入 URL / Scheme"
              value={form.quickImportUrl}
              errors={errors.quick_import_url}
              description="必须包含 {url} 或 {base64url}，可选 {name}。"
              onChange={(quickImportUrl) =>
                setForm({ ...form, quickImportUrl })
              }
            />
          ) : null}
          <FieldSet className="rounded-2xl border p-4">
            <FieldLegend>设备平台</FieldLegend>
            <FieldDescription>
              至少选择一个，且同一设备/平台不可重复。
            </FieldDescription>
            <div className="grid gap-3 sm:grid-cols-2">
              {Object.entries(devicePlatforms).map(([device, platforms]) => (
                <div key={device} className="rounded-xl border p-3">
                  ...
                  <div className="space-y-2">
                    {platforms.map((platform) => (
                      ...
                      >
                        <Checkbox
                          checked={hasScope(device, platform)}
                          onCheckedChange={(checked) =>
                            toggleScope(device, platform, checked === true)
                          }
                        />
            ...
            <FieldError
              errors={errors.scopes?.map((message) => ({ message }))}
            />
          </FieldSet>
        </FieldGroup>
        <DialogFooter>
          ...
          <Button
            disabled={
              ...
              !form.scopes.length ||
              (form.logoMode === 'url' && !form.logoUrl.trim()) ||
              (form.quickImportEnabled && !form.quickImportUrl.trim())
            }
            onClick={onSave}
          >
...
function idsForScope(clients: Client[], device: string, platform: string) {
  return clients
    .filter((client) =>
      client.scopes.some(
        (scope) => scope.device_type === device && scope.platform === platform,
...
function move(ids: number[], index: number, offset: -1 | 1) {
